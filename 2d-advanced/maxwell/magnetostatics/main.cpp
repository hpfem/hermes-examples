...
int main(int argc, char* argv[])
{
  ...
  std::vector<double> mu_inv_pts(0.0, 0.5, 0.9, 1.0, 1.1, 1.2, 1.3,
    1.4, 1.6, 1.7, 1.8, 1.9, 3.0, 5.0, 10.0);
  std::vector<double> mu_inv_val(1 / 1500.0, 1 / 1480.0, 1 / 1440.0, 1 / 1400.0, 1 / 1300.0, 1 / 1150.0, 1 / 950.0,
  ...
  MeshFunctionSharedPtr<double> vector_potential(new FilterVectorPotential({sln, sln},
    std::vector<int>(H2D_FN_VAL, H2D_FN_VAL)));
  s_view1.show_mesh(false);
  s_view1.show(vector_potential);

  ScalarView s_view2("Flux density", new WinGeom(360, 0, 350, 450));
  MeshFunctionSharedPtr<double> flux_density(new FilterFluxDensity({sln, sln}));
