...
int main(int argc, char* argv[])
{
  ...
  DefaultEssentialBCConst<double> bc_other_vel_x({"Outer Wall", "Inner Wall"}, 0.0);
  EssentialBCs<double> bcs_vel_x({&bc_inlet_vel_x, &bc_other_vel_x});
  DefaultEssentialBCConst<double> bc_vel_y({"Inlet", "Outer Wall", "Inner Wall"}, 0.0);
  EssentialBCs<double> bcs_vel_y(&bc_vel_y);
  EssentialBCs<double> bcs_pressure;
  DefaultEssentialBCConst<double> bc_temperature({"Outer Wall", "Inlet"}, 20.0);
  ...
  int ndof = Space<double>::get_num_dofs({xvel_space,
  ...
  std::vector<NormType> all_proj_norms = std::vector<NormType>(vel_proj_norm,
  ...
  std::vector<MeshFunctionSharedPtr<double> > initial_solutions = std::vector<MeshFunctionSharedPtr<double> >(xvel_prev_time,
    yvel_prev_time, p_prev_time, temperature_init_cond);
  std::vector<MeshFunctionSharedPtr<double> > all_solutions = std::vector<MeshFunctionSharedPtr<double> >(xvel_prev_time,
